(function () {
const STORAGE_KEY = "guchirin-lang";
const SUPPORTED_LANGS = ["ja", "en", "ko", "zh"];
const FALLBACK_LANG = "en";

const DICT = {
  ja: {
    page_title: "ぐちりん",
    new_chat_btn: "+ 新しいチャット",
    default_session_title: "新しいチャット",
    memory_toggle_btn: "記憶パネル",
    profile_toggle_btn: "プロフィール",
    progress_toggle_btn: "進行状況",
    settings_toggle_btn: "設定",
    lang_label: "言語",
    lang_auto: "自動（システム設定）",
    lang_ja: "日本語",
    lang_en: "English",
    lang_ko: "한국어",
    lang_zh: "中文",
    model_label: "モデル",
    model_not_downloaded_suffix: "（未ダウンロード）",
    model_download_progress: "「{model}」をダウンロード中: {status}",
    model_download_done: "「{model}」のダウンロードが完了しました",
    model_download_error: "ダウンロードに失敗しました: {error}",
    search_mode_label: "検索モード",
    message_input_placeholder: "メッセージを入力...",
    send_btn: "送信",
    stopped_message: "（停止しました）",
    memory_panel_title: "記憶",
    memory_add_placeholder: "記憶を追加...",
    memory_add_btn: "追加",
    memory_status_count: "{count} / {max} 件",
    memory_status_full: "（容量満杯：新規追加不可、更新のみ）",
    memory_status_warning: "（容量警告）",
    memory_full_alert: "記憶の容量が満杯のため追加できません",
    profile_panel_title: "プロフィール",
    basic_info_title: "基本情報",
    ph_name: "氏名",
    ph_birth_date: "生年月日",
    ph_current_company: "現在の勤務先",
    ph_current_position: "現在の職位",
    ph_current_salary: "現在の年収",
    sensitive_info_title: "機密性の高い個人情報（任意）",
    ph_gender: "性別",
    ph_religion: "宗教",
    ph_sexual_orientation: "性的志向",
    ph_race: "人種",
    ph_hometown: "出身地",
    ph_residence_country: "居住国",
    ph_nationality: "国籍",
    residence_title: "居住歴",
    residence_add_toggle: "+ 居住歴を追加",
    ph_residence_place: "国・都市",
    ph_residence_period: "期間（例: 2010〜2015、2012/4〜2015/3）",
    residence_add_submit: "この居住歴を追加",
    save_btn: "保存",
    career_title: "職歴",
    career_add_toggle: "+ 職歴を追加",
    ph_company: "会社名",
    ph_position: "職位",
    ph_start_date: "開始(YYYY-MM)",
    ph_end_date: "終了(YYYY-MM、在籍中は空欄)",
    ph_salary: "収入",
    ph_reason_joining: "入社理由",
    ph_reason_leaving: "退職理由",
    ph_note: "自由記述（その他のエピソードなど）",
    career_add_submit: "この職歴を追加",
    education_title: "学歴",
    education_add_toggle: "+ 学歴を追加",
    ph_degree: "学位（学士/修士/博士など）",
    ph_field: "専攻",
    ph_school: "学校名",
    ph_graduated_year: "卒業年（YYYY）",
    education_add_submit: "この学歴を追加",
    import_title: "職務経歴書 / LinkedIn から一括登録",
    import_textarea_placeholder: "職務経歴書やLinkedInプロフィールのテキストを貼り付け...",
    import_submit: "読み込んで登録",
    import_submit_loading: "読み込み中...",
    import_file_submit: "ファイルから読み込んで登録",
    import_file_failed: "読み込みに失敗しました",
    progress_panel_title: "進行状況",
    progress_thinking_label: "思考中...",
    progress_tool_web_search: "Web検索",
    progress_tool_retrieve_memory: "記憶検索",
    progress_tool_retrieve_profile: "プロフィール検索(SQL)",
    settings_panel_title: "設定",
    settings_context_window: "コンテキストウィンドウ(num_ctx)",
    settings_answer_max_tokens: "最終回答の最大トークン数",
    settings_plan_max_tokens: "プランニングの最大トークン数",
    settings_light_task_max_tokens: "軽量タスクの最大トークン数",
    settings_import_max_tokens: "経歴書インポートの最大トークン数",
    settings_max_history_messages: "履歴として渡す直近メッセージ数",
    settings_answer_style_prompt: "回答スタイルの指示",
    settings_reset_btn: "デフォルトに戻す",
  },
  en: {
    page_title: "Guchirin",
    new_chat_btn: "+ New Chat",
    default_session_title: "New Chat",
    memory_toggle_btn: "Memory Panel",
    profile_toggle_btn: "Profile",
    progress_toggle_btn: "Progress",
    settings_toggle_btn: "Settings",
    lang_label: "Language",
    lang_auto: "Auto (system)",
    lang_ja: "日本語",
    lang_en: "English",
    lang_ko: "한국어",
    lang_zh: "中文",
    model_label: "Model",
    model_not_downloaded_suffix: " (not downloaded)",
    model_download_progress: "Downloading \"{model}\": {status}",
    model_download_done: "Finished downloading \"{model}\"",
    model_download_error: "Download failed: {error}",
    search_mode_label: "Search Mode",
    message_input_placeholder: "Type a message...",
    send_btn: "Send",
    stopped_message: "(Stopped)",
    memory_panel_title: "Memory",
    memory_add_placeholder: "Add a memory...",
    memory_add_btn: "Add",
    memory_status_count: "{count} / {max} entries",
    memory_status_full: "(Full: no new entries, updates only)",
    memory_status_warning: "(Capacity warning)",
    memory_full_alert: "Cannot add: memory capacity is full",
    profile_panel_title: "Profile",
    basic_info_title: "Basic Info",
    ph_name: "Name",
    ph_birth_date: "Birth date",
    ph_current_company: "Current company",
    ph_current_position: "Current position",
    ph_current_salary: "Current salary",
    sensitive_info_title: "Sensitive personal info (optional)",
    ph_gender: "Gender",
    ph_religion: "Religion",
    ph_sexual_orientation: "Sexual orientation",
    ph_race: "Race/ethnicity",
    ph_hometown: "Hometown",
    ph_residence_country: "Country of residence",
    ph_nationality: "Nationality",
    residence_title: "Residence history",
    residence_add_toggle: "+ Add residence entry",
    ph_residence_place: "Country/city",
    ph_residence_period: "Period (e.g. 2010-2015, 2012/04-2015/03)",
    residence_add_submit: "Add this entry",
    save_btn: "Save",
    career_title: "Career History",
    career_add_toggle: "+ Add career entry",
    ph_company: "Company",
    ph_position: "Position",
    ph_start_date: "Start (YYYY-MM)",
    ph_end_date: "End (YYYY-MM, blank if current)",
    ph_salary: "Salary",
    ph_reason_joining: "Reason for joining",
    ph_reason_leaving: "Reason for leaving",
    ph_note: "Notes (other details, etc.)",
    career_add_submit: "Add this entry",
    education_title: "Education",
    education_add_toggle: "+ Add education entry",
    ph_degree: "Degree (Bachelor's, Master's, etc.)",
    ph_field: "Field of study",
    ph_school: "School name",
    ph_graduated_year: "Graduation year (YYYY)",
    education_add_submit: "Add this entry",
    import_title: "Bulk import from resume / LinkedIn",
    import_textarea_placeholder: "Paste resume or LinkedIn profile text...",
    import_submit: "Import & Register",
    import_submit_loading: "Loading...",
    import_file_submit: "Import from file",
    import_file_failed: "Import failed",
    progress_panel_title: "Progress",
    progress_thinking_label: "Thinking...",
    progress_tool_web_search: "Web search",
    progress_tool_retrieve_memory: "Memory search",
    progress_tool_retrieve_profile: "Profile search (SQL)",
    settings_panel_title: "Settings",
    settings_context_window: "Context window (num_ctx)",
    settings_answer_max_tokens: "Final answer max tokens",
    settings_plan_max_tokens: "Planning max tokens",
    settings_light_task_max_tokens: "Light task max tokens",
    settings_import_max_tokens: "Resume import max tokens",
    settings_max_history_messages: "Recent messages kept in history",
    settings_answer_style_prompt: "Answer style instructions",
    settings_reset_btn: "Reset to defaults",
  },
  ko: {
    page_title: "구치링",
    new_chat_btn: "+ 새 채팅",
    default_session_title: "새 채팅",
    memory_toggle_btn: "기억 패널",
    profile_toggle_btn: "프로필",
    progress_toggle_btn: "진행 상황",
    settings_toggle_btn: "설정",
    lang_label: "언어",
    lang_auto: "자동(시스템 설정)",
    lang_ja: "日本語",
    lang_en: "English",
    lang_ko: "한국어",
    lang_zh: "中文",
    model_label: "모델",
    model_not_downloaded_suffix: "（다운로드 안 됨）",
    model_download_progress: "「{model}」다운로드 중: {status}",
    model_download_done: "「{model}」다운로드가 완료되었습니다",
    model_download_error: "다운로드에 실패했습니다: {error}",
    search_mode_label: "검색 모드",
    message_input_placeholder: "메시지를 입력...",
    send_btn: "보내기",
    stopped_message: "(중지됨)",
    memory_panel_title: "기억",
    memory_add_placeholder: "기억 추가...",
    memory_add_btn: "추가",
    memory_status_count: "{count} / {max} 개",
    memory_status_full: "(용량 가득 참: 신규 추가 불가, 수정만 가능)",
    memory_status_warning: "(용량 경고)",
    memory_full_alert: "기억 용량이 가득 차서 추가할 수 없습니다",
    profile_panel_title: "프로필",
    basic_info_title: "기본 정보",
    ph_name: "이름",
    ph_birth_date: "생년월일",
    ph_current_company: "현재 직장",
    ph_current_position: "현재 직위",
    ph_current_salary: "현재 연봉",
    sensitive_info_title: "민감한 개인정보（선택）",
    ph_gender: "성별",
    ph_religion: "종교",
    ph_sexual_orientation: "성적 지향",
    ph_race: "인종",
    ph_hometown: "출신지",
    ph_residence_country: "거주국",
    ph_nationality: "국적",
    residence_title: "거주 이력",
    residence_add_toggle: "+ 거주 이력 추가",
    ph_residence_place: "국가・도시",
    ph_residence_period: "기간(예: 2010~2015, 2012/4~2015/3)",
    residence_add_submit: "이 거주 이력 추가",
    save_btn: "저장",
    career_title: "경력",
    career_add_toggle: "+ 경력 추가",
    ph_company: "회사명",
    ph_position: "직위",
    ph_start_date: "시작(YYYY-MM)",
    ph_end_date: "종료(YYYY-MM, 재직 중이면 공란)",
    ph_salary: "연봉",
    ph_reason_joining: "입사 이유",
    ph_reason_leaving: "퇴사 이유",
    ph_note: "자유 기술(기타 에피소드 등)",
    career_add_submit: "이 경력 추가",
    education_title: "학력",
    education_add_toggle: "+ 학력 추가",
    ph_degree: "학위(학사/석사/박사 등)",
    ph_field: "전공",
    ph_school: "학교명",
    ph_graduated_year: "졸업 연도(YYYY)",
    education_add_submit: "이 학력 추가",
    import_title: "경력서 / LinkedIn 일괄 등록",
    import_textarea_placeholder: "경력서나 LinkedIn 프로필 텍스트를 붙여넣기...",
    import_submit: "불러와서 등록",
    import_submit_loading: "불러오는 중...",
    import_file_submit: "파일에서 불러와서 등록",
    import_file_failed: "불러오기에 실패했습니다",
    progress_panel_title: "진행 상황",
    progress_thinking_label: "생각 중...",
    progress_tool_web_search: "웹 검색",
    progress_tool_retrieve_memory: "기억 검색",
    progress_tool_retrieve_profile: "프로필 검색(SQL)",
    settings_panel_title: "설정",
    settings_context_window: "컨텍스트 윈도우(num_ctx)",
    settings_answer_max_tokens: "최종 답변 최대 토큰 수",
    settings_plan_max_tokens: "플래닝 최대 토큰 수",
    settings_light_task_max_tokens: "경량 작업 최대 토큰 수",
    settings_import_max_tokens: "경력서 가져오기 최대 토큰 수",
    settings_max_history_messages: "전달할 최근 메시지 수",
    settings_answer_style_prompt: "답변 스타일 지침",
    settings_reset_btn: "기본값으로 재설정",
  },
  zh: {
    page_title: "居期林",
    new_chat_btn: "+ 新建聊天",
    default_session_title: "新建聊天",
    memory_toggle_btn: "记忆面板",
    profile_toggle_btn: "个人资料",
    progress_toggle_btn: "进度",
    settings_toggle_btn: "设置",
    lang_label: "语言",
    lang_auto: "自动（系统设置）",
    lang_ja: "日本語",
    lang_en: "English",
    lang_ko: "한국어",
    lang_zh: "中文",
    model_label: "模型",
    model_not_downloaded_suffix: "（未下载）",
    model_download_progress: "正在下载「{model}」: {status}",
    model_download_done: "「{model}」下载完成",
    model_download_error: "下载失败: {error}",
    search_mode_label: "搜索模式",
    message_input_placeholder: "输入消息...",
    send_btn: "发送",
    stopped_message: "（已停止）",
    memory_panel_title: "记忆",
    memory_add_placeholder: "添加记忆...",
    memory_add_btn: "添加",
    memory_status_count: "{count} / {max} 条",
    memory_status_full: "（容量已满：无法新增，仅可更新）",
    memory_status_warning: "（容量警告）",
    memory_full_alert: "记忆容量已满，无法添加",
    profile_panel_title: "个人资料",
    basic_info_title: "基本信息",
    ph_name: "姓名",
    ph_birth_date: "出生日期",
    ph_current_company: "当前公司",
    ph_current_position: "当前职位",
    ph_current_salary: "当前薪资",
    sensitive_info_title: "敏感个人信息（可选）",
    ph_gender: "性别",
    ph_religion: "宗教",
    ph_sexual_orientation: "性取向",
    ph_race: "种族",
    ph_hometown: "籍贯",
    ph_residence_country: "居住国",
    ph_nationality: "国籍",
    residence_title: "居住历史",
    residence_add_toggle: "+ 添加居住记录",
    ph_residence_place: "国家・城市",
    ph_residence_period: "期间（例如: 2010-2015、2012/4-2015/3）",
    residence_add_submit: "添加此记录",
    save_btn: "保存",
    career_title: "工作经历",
    career_add_toggle: "+ 添加工作经历",
    ph_company: "公司名称",
    ph_position: "职位",
    ph_start_date: "开始(YYYY-MM)",
    ph_end_date: "结束(YYYY-MM，在职则留空)",
    ph_salary: "薪资",
    ph_reason_joining: "入职原因",
    ph_reason_leaving: "离职原因",
    ph_note: "备注（其他细节等）",
    career_add_submit: "添加此经历",
    education_title: "教育经历",
    education_add_toggle: "+ 添加教育经历",
    ph_degree: "学位（学士/硕士/博士等）",
    ph_field: "专业",
    ph_school: "学校名称",
    ph_graduated_year: "毕业年份（YYYY）",
    education_add_submit: "添加此经历",
    import_title: "从简历 / LinkedIn 批量导入",
    import_textarea_placeholder: "粘贴简历或LinkedIn个人资料文本...",
    import_submit: "导入并注册",
    import_submit_loading: "加载中...",
    import_file_submit: "从文件导入",
    import_file_failed: "导入失败",
    progress_panel_title: "进度",
    progress_thinking_label: "思考中...",
    progress_tool_web_search: "网络搜索",
    progress_tool_retrieve_memory: "记忆检索",
    progress_tool_retrieve_profile: "个人资料检索(SQL)",
    settings_panel_title: "设置",
    settings_context_window: "上下文窗口(num_ctx)",
    settings_answer_max_tokens: "最终回答最大令牌数",
    settings_plan_max_tokens: "规划最大令牌数",
    settings_light_task_max_tokens: "轻量任务最大令牌数",
    settings_import_max_tokens: "简历导入最大令牌数",
    settings_max_history_messages: "传递的最近消息数",
    settings_answer_style_prompt: "回答风格说明",
    settings_reset_btn: "恢复默认设置",
  },
};

function detectSystemLang() {
  const langs = navigator.languages && navigator.languages.length ? navigator.languages : [navigator.language];
  for (const raw of langs) {
    const base = (raw || "").slice(0, 2).toLowerCase();
    if (SUPPORTED_LANGS.includes(base)) return base;
  }
  return FALLBACK_LANG;
}

function getLangSetting() {
  return localStorage.getItem(STORAGE_KEY) || "auto";
}

function setLangSetting(value) {
  localStorage.setItem(STORAGE_KEY, value);
}

function getEffectiveLang() {
  const setting = getLangSetting();
  return setting === "auto" ? detectSystemLang() : setting;
}

function t(key, vars) {
  const lang = getEffectiveLang();
  const entry = DICT[lang] || DICT[FALLBACK_LANG];
  let text = entry[key] ?? DICT[FALLBACK_LANG][key] ?? key;
  if (vars) {
    for (const [k, v] of Object.entries(vars)) {
      text = text.replace(`{${k}}`, v);
    }
  }
  return text;
}

function applyTranslations() {
  document.documentElement.lang = getEffectiveLang();
  document.title = t("page_title");
  document.querySelectorAll("[data-i18n]").forEach((el) => {
    el.textContent = t(el.dataset.i18n);
  });
  document.querySelectorAll("[data-i18n-placeholder]").forEach((el) => {
    el.placeholder = t(el.dataset.i18nPlaceholder);
  });
}

window.I18N = { t, getLangSetting, setLangSetting, getEffectiveLang, applyTranslations };
})();
